...
const STAGES = [
  ...
  {
    key: "input",
    label: "Scrub the question",
    detail: "Remove private data · block sneaky prompts",
  },
  { key: "model", label: "Ask the AI", detail: "Send the clean prompt to your chosen model" },
  {
    key: "judge",
    label: "Cross-check the answer",
    detail: "Does what the AI said match your policies?",
  },
...
const BLOCK_AT: Record<
  Scenario,
  { idx: number; state: State; reason: string; rule: string } | null
> = {
  clean: null,
  pii: {
    idx: 1,
...
const PAYLOADS: Record<Scenario, Record<number, { before: string; after: string }>> = {
  clean: {
    1: {
      before: `{ "prompt": "Summarize Q3 fund performance vs benchmark" }`,
      after: `{ "prompt": "Summarize Q3 fund performance vs benchmark", "scrub": [] }`,
    },
    2: {
      before: `POST → gemini-1.5-pro`,
      after: `{ "completion": "Q3 outperformance was 3.2%…", "tokens": 214 }`,
    },
    3: {
      before: `judge(completion, policies=[SEC-206(4)-1])`,
      after: `{ "mismatch": false, "confidence": 0.94 }`,
    },
    4: {
      before: `enforce_citations(response)`,
      after: `{ "coverage": 0.92, "cited": ["SEC-206(4)-1","FINRA-2210"] }`,
    },
    5: {
      before: `append_ledger(run)`,
      after: `{ "id": "jc_00817", "hash": "0xa1f…", "verdict": "allow" }`,
    },
  },
  pii: {
    1: {
      before: `{ "prompt": "…client SSN 123-45-6789…" }`,
      after: `BLOCKED — PII detected, request rejected`,
    },
    2: { before: "-", after: "-" },
    3: { before: "-", after: "-" },
    4: { before: "-", after: "-" },
    5: {
      before: "append_ledger(block)",
      after: `{ "id": "jc_00818", "verdict": "block", "rule": "SEC-T42" }`,
    },
  },
  drift: {
    1: {
      before: `{ "prompt": "Draft PR summary for fee change" }`,
      after: `{ "prompt": "…", "scrub": [] }`,
    },
    2: {
      before: `POST → claude-3.5-sonnet`,
      after: `{ "completion": "Cross-border fee remains 1%…", "tokens": 187 }`,
    },
    3: {
      before: `judge(completion, git_diff, sec_filing)`,
      after: `BLOCKED — draft contradicts SEC filing (line 42)`,
    },
    4: { before: "-", after: "-" },
    5: {
      before: "append_ledger(block)",
      after: `{ "id": "jc_00819", "verdict": "block", "rule": "SEC-206(4)-1" }`,
    },
  },
  uncited: {
    1: {
      before: `{ "prompt": "Explain suitability for retiree" }`,
      after: `{ "prompt": "…", "scrub": [] }`,
    },
    ...
    4: {
      before: `enforce_citations(response)`,
      after: `REVISE — 3 uncited "must/required" claims`,
    },
    5: {
      before: "append_ledger(revise)",
      after: `{ "id": "jc_00820", "verdict": "revise", "rule": "FINRA-2210" }`,
    },
  },
};
      ...
      const t2 = setTimeout(
        () => {
          ...
          if (blocker && blocker.idx === i) {
            setAnnounce(
              `Stage ${i + 1}: ${s.label} · ${blocker.state === "block" ? "blocked" : "revised"} — ${blocker.reason}`,
            );
            setRunning(false);
          }
        },
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <label htmlFor="pl-scenario" className="sr-only">
              Scenario
            </label>
            <Select
              value={scenario}
            ...
            >
              <SelectTrigger id="pl-scenario" className="w-72">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <Button onClick={play} disabled={running || killSwitch}>
            {killSwitch ? (
              ...
              <>
                <Play className="h-4 w-4 mr-2" /> {running ? "Running…" : "Play scenario"}
              </>
            )}
          </Button>
        </CardHeader>
        <CardContent>
          <ol className="grid grid-cols-1 md:grid-cols-6 gap-3" aria-label="Pipeline stages">
            {STAGES.map((s, i) => {
              ...
              const color =
                state === "block"
                  ? "border-[color:var(--block)] bg-[color:var(--block)]/10 text-[color:var(--block)]"
                  : state === "revise"
                    ? "border-[color:var(--revise)] bg-[color:var(--revise)]/10 text-[color:var(--revise)]"
                    : state === "pass"
                      ? "border-[color:var(--allow)]/60 bg-[color:var(--allow)]/10 text-[color:var(--allow)]"
                      : state === "active"
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid md:grid-cols-2 gap-3">
            <div>
              ...
              <pre className="rounded-md border border-border bg-muted/20 p-3 font-mono text-xs overflow-x-auto whitespace-pre-wrap">
                {payload.before}
              </pre>
            </div>
            <div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Scenario verdict</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            {!blocker ? (
              <>
                <Badge
                  variant="outline"
                  className="border-[color:var(--allow)]/40 text-[color:var(--allow)]"
                >
                  ALLOW
                </Badge>
                <p className="text-muted-foreground text-xs">
                  All six stages passed. Ledger entry appended.
                </p>
              </>
            ) : (
              <>
                ...
                >
                  {blocker.state.toUpperCase()}
                </Badge>
                <p className="text-xs text-muted-foreground">
                  Stopped at stage {blocker.idx + 1}:{" "}
                  <span className="text-foreground">{STAGES[blocker.idx].label}</span>
                </p>
