  ...
  return (
    <div className="min-h-dvh p-0 md:p-4 lg:p-6" style={{ background: "var(--app-bg)" }}>
      ...
      <aside aria-label="Dashboard navigation" className="w-full md:w-64 border-b md:border-b-0 md:border-r border-border/60 bg-sidebar shrink-0 flex md:flex-col">
        ...
        <nav aria-label="Dashboard sections" className="p-3 flex-1 flex md:block gap-1 md:space-y-4 overflow-x-auto">
          {groups.map((g) => (
            <div key={g.label} className="md:space-y-1">
              <div className="hidden md:block px-2 pb-1 text-[10px] uppercase tracking-wider text-muted-foreground/70 font-medium">{g.label}</div>
              {g.items.map((n) => {
                const active = n.exact ? pathname === n.to : pathname === n.to || (n.to !== "/dashboard" && pathname.startsWith(n.to + "/"));
                return (
                  <Link
                    key={n.to}
                    to={n.to}
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="h-14 border-b border-border/60 bg-card/40 backdrop-blur-sm flex items-center gap-3 px-4 sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-3">
            {killSwitch && (
              <Badge variant="outline" className="border-[color:var(--block)]/50 text-[color:var(--block)] gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-[color:var(--block)] shield-pulse" aria-hidden />
                LOCKDOWN
              </Badge>
            )}
