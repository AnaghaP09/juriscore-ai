...
import { AUDIT, DEMO_MODELS, RULEBOOKS, getMetrics } from "@/lib/juriscore/mock";
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Goal: at least 85% (checked by humans)
            </div>
          </CardContent>
        </Card>
        <Card>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {RULEBOOKS.reduce((s, r) => s + r.docCount, 0)} documents indexed
                </div>
              </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-2 text-3xl font-semibold font-mono text-[color:var(--block)]">
                  {mismatches.length}
                </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  Well under the 800ms budget
                </div>
              </div>
              <Timer className="h-5 w-5 text-primary" aria-hidden />
            </div>
            <Sparkline
              data={[520, 480, 510, 460, 500, m.kpis.avgLatencyMs, m.kpis.avgLatencyMs - 20]}
            />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">AI health by region</CardTitle>
          </CardHeader>
            ...
            <table className="w-full text-sm" aria-label="Model endpoint health matrix">
              <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
                <tr>
                  ...
                  {REGIONS.map((r) => (
                    <th key={r} scope="col" className="text-left px-4 py-2 font-medium font-mono">
                      {r}
                    </th>
                  ))}
                </tr>
                      ...
                      const color =
                        status === "Blocked"
                          ...
                          : status === "Degraded"
                            ...
                            : "border-[color:var(--allow)]/40 text-[color:var(--allow)]";
                      return (
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">Things we caught, still open</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {mismatches.map((mm) => (
              ...
              >
                <span
                  className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${mm.verdict === "block" ? "bg-[color:var(--block)]" : "bg-[color:var(--revise)]"}`}
                  aria-hidden
                />
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground truncate">
                    {mm.reason ?? mm.useCase}
                  </p>
                </div>
                <span className="font-mono text-xs text-primary shrink-0">
                  {mm.retrievedPolicyIds[0]}
                </span>
              </div>
            ))}
          </CardContent>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ShieldAlert className="h-5 w-5 text-[color:var(--block)]" /> Freeze all AI right now?
            </DialogTitle>
            <DialogDescription>
              This immediately stops every AI request across every model and region. Nothing will
              get an answer until you turn it back on. Use this in a live incident only.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
function RadialDial({ value, target }: { value: number; target: number }) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`Drift precision ${(pct * 100).toFixed(1)} percent`}
    >
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle
          ...
          cy={size / 2}
          r={r}
          stroke={above ? "var(--allow)" : "var(--revise)"}
          strokeWidth={stroke}
          fill="none"
          strokeDasharray={c}
      ...
      <div>
        ...
        <div
          className={`text-xs font-mono ${above ? "text-[color:var(--allow)]" : "text-[color:var(--revise)]"}`}
        >
