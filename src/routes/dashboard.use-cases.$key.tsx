...
import { AUDIT, getUseCaseSummaries, type UseCaseSummary } from "@/lib/juriscore/mock";
import {
  downloadCSV,
  openPrintReport,
  kpiCard,
  ...
  head: ({ params }) => ({
    meta: [
      { title: `${params.key} — Use case · JurisCore AI` },
      {
        name: "description",
        content:
          "Drill-down metrics, failure modes and recent audit trail for a governed use case.",
      },
    ],
  }),
  loader: ({ params }): { uc: UseCaseSummary } => {
...
function NotFoundView() {
  return (
    <div className="p-8">
      ...
      <Link to="/dashboard/use-cases" className="text-primary text-sm underline">
        Back to use cases
      </Link>
    </div>
  );
  ...
  return (
    <div className="p-8 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <Link to="/dashboard/use-cases" className="text-primary text-sm underline self-center">
          Back to use cases
        </Link>
      </div>
    </div>
...
function UseCaseDetail() {
  ...
  const uc = getUseCaseSummaries().find((useCase) => useCase.key === key);
  if (!uc) return <NotFoundView />;
  const entries = AUDIT.filter((a) => a.useCase === uc.key);
  const recent = entries.slice(0, 25);
  const avgLatency = entries.length
    ? Math.round(entries.reduce((s, a) => s + a.latencyMs, 0) / entries.length)
    : 0;
  ...
  const exportCSV = () => {
    downloadCSV(
      ...
      entries.map((e) => ({
        ...
        blockedStage: e.blockedStage ?? "",
        reason: e.reason ?? "",
        citationCoverage: e.citationCoverage,
        policies: e.retrievedPolicyIds.join("|"),
        prompt: e.prompt,
      })),
    );
  };
          ...
          ? [
              {
                ...
                html: `<table>${[
                  ...
                  uc.capability &&
                    `<tr><th>Capability</th><td>${escapeHtml(uc.capability)}</td></tr>`,
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6">
      ...
      <PageHeader
          ...
          <span className="flex items-center gap-2 flex-wrap normal-case tracking-normal">
            ...
            {isPlumb && (
              <Badge variant="outline" className="border-primary/50 text-primary gap-1">
                <GitPullRequest className="h-3 w-3" aria-hidden /> Plumb wedge
              </Badge>
            )}
          </span>
        }
        title={uc.name}
        description={
          uc.tagline ?? "What this AI feature does, how often it runs, and when we had to step in."
        }
      />
        ...
        <Card>
          <CardContent className="p-8 text-center space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Once JurisCore processes requests for {uc.name}, metrics and receipts will appear
              here.
            </p>
            <Link to="/dashboard/use-cases" className="text-primary text-sm underline inline-block">
              Back to use cases
            </Link>
          </CardContent>
        </Card>
        ...
        <>
          {entries.length < 10 && (
            <div className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground flex items-center gap-2">
              <Info className="h-3.5 w-3.5" aria-hidden /> Low sample size ({entries.length}{" "}
              entries) — metrics may not be representative.
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <MetricCard
              label="Block rate"
              value={`${(uc.blockRate * 100).toFixed(1)}%`}
              tone={uc.blockRate > 0.1 ? "block" : "allow"}
            />
            <MetricCard
              label="Citation coverage"
              value={`${(uc.citationCoverage * 100).toFixed(0)}%`}
              tone={uc.citationCoverage > 0.8 ? "allow" : "revise"}
            />
          ...
          {(uc.persona || uc.capability || uc.valueProp || uc.risk) && (
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {uc.valueProp && (
                <ProfileCard title="Value delivered" body={uc.valueProp} tone="allow" />
              )}
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="lg:col-span-1">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">Verdict mix</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <VerdictBar
                  label="Allowed"
                  count={entries.length - blocked - revised}
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">Top failure modes</CardTitle>
              </CardHeader>
              <CardContent>
                {topReasons.length === 0 && (
                  <p className="text-sm text-muted-foreground">
                    No blocked or revised entries in window.
                  </p>
                )}
                  ...
                  <tbody>
                    {recent.map((r) => (
                      <tr key={r.id} className="border-t border-border/60">
                        <td className="px-4 py-2 font-mono text-xs truncate max-w-[10ch]">
                          {r.id}
                        </td>
...
function ProfileCard({
  ...
  const border =
    tone === "allow"
      ? "border-[color:var(--allow)]/30"
      : tone === "revise"
        ? "border-[color:var(--revise)]/30"
        : "border-border";
  return (
